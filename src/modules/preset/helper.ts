...
export const updatePreset = async (data: any, userId?: string) => {
  ...
  let response = null;
  const corpusDecoded = getCorpusDecoded(data.type, data.corpus);
  if (data._id) {
    response = await model.findByIdAndUpdate(
      data._id, { ...data, corpusDecoded, updatedBy: userId },
      { new: true, upsert: true }
    );
  } else {
    response = await model.create({ ...data, corpusDecoded, createdBy: userId, updatedBy: userId });
  }
  return await getPreset(userId);
};

const getCorpusDecoded = (type: 'Word' | 'Sentence' | 'AI', corpus: string) => {
  switch (type) {
    case "Word":
